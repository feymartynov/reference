...
impl RwLockBTreeMapUpdater {
    fn start(ids: Arc<RwLock<BTreeMap<Id, usize>>>) -> Self {
...
struct RwLockHashUpdater {
    is_halt: Arc<AtomicBool>,
}

impl RwLockHashUpdater {
    fn start<H>(ids: Arc<RwLock<HashMap<Id, usize, BuildHasherDefault<H>>>>) -> Self
    where
...
impl Drop for RwLockHashUpdater {
    fn drop(&mut self) {
        self.is_halt.store(true, Ordering::SeqCst);
    }
}

fn id_index_rwlock_hash<H: Hasher + Default + 'static>(bencher: &mut Bencher) {
    ...
    let ids = Arc::new(RwLock::new(ids));
    let _updater = RwLockHashUpdater::start(ids.clone());
    let mut rng = rand::thread_rng();
...
benchmark_group!(
    ...
    id_index_rwlock_hash::<DefaultHasher>,
    id_index_rwlock_hash::<FxHasher>,
    id_index_rwlock_hash::<NoHashHasher<Id>>,
